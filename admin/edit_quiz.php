...
try {
    ...
    // Structure Lock: Prevent Adding/Deleting questions if submissions exist
    $is_structure_locked = ($participation_count > 0);
    // Content Lock: Previously suppressed editing. Now we ALLOW editing text/prompts.
    $can_edit_content = true; // Always allow editing text/prompts/time
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // STRICT SECURITY: Prevent Structural Changes if Locked
    if ($is_structure_locked) {
        // Block Deletion
        if (isset($_POST['action']) && $_POST['action'] === 'delete_question') {
             show_message('Sınavda aktif katılımlar olduğu için soru SİLEMEZSİNİZ.', 'danger');
             redirect("edit_quiz.php?id=$quiz_id");
             exit;
        }
        // Block AI Generation (Adds questions)
        if (isset($_POST['action']) && $_POST['action'] === 'generate_ai') {
             show_message('Sınavda aktif katılımlar olduğu için yeni soru EKLEYEMEZSİNİZ.', 'danger');
             redirect("edit_quiz.php?id=$quiz_id");
             exit;
        }
        // Block Manual Insertion (Check for new IDs)
        ...
        <div class="col-lg-10">
            <div class="card shadow">
                ...
                <div class="card-body">
                    ...
                    <?php if ($is_structure_locked): ?>
                        <div class="alert alert-info shadow-sm border-start border-4 border-info">
                            <i class="fas fa-info-circle me-2"></i>
                            <strong>Bilgi:</strong> Bu sınava <b><?php echo $participation_count; ?></b> öğrenci katılmıştır.
                            Soru metinlerini, süreleri ve AI yönergelerini düzenleyebilirsiniz. Ancak <u>soru ekleyemez, silemez veya türünü değiştiremezsiniz.</u>
                        </div>
                    <?php endif; ?>
                                ...
                                <div class="card mb-3 border question-item" id="q_card_<?php echo $index; ?>">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row mb-2">
                                            ...
                                            <div class="col-md-3">
                                                 <select name="questions[<?php echo $index; ?>][type]" class="form-select q-type-select" data-index="<?php echo $index; ?>" <?php echo $is_structure_locked ? 'disabled' : ''; ?>>
