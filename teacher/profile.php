...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_email'])) {
        ...
        if (filter_var($new_email, FILTER_VALIDATE_EMAIL)) {
            ...
            $check_stmt->bindParam(':user_id', $teacher_id);
            $check_stmt->execute();
            ...
            if ($check_stmt->rowCount() == 0) {
                ...
                $update_stmt->bindParam(':user_id', $teacher_id);
    ...
    if (isset($_POST['upload_image']) && isset($_FILES['profile_image'])) {
        ...
        if ($file['error'] == 0) {
            ...
            if (in_array($file['type'], $allowed_types) && $file['size'] <= $max_size) {
                ...
                $custom_name = 'profile_' . $teacher_id . '_' . time();
...
// Öğretmenin ders istatistikleri
try {
    $query = "SELECT 
                COUNT(c.id) as total_courses,
                (SELECT COUNT(*) FROM course_enrollments ce 
                 JOIN courses c2 ON ce.course_id = c2.id 
                 WHERE c2.teacher_id = :teacher_id AND ce.is_active = 1) as total_students,
                (SELECT COUNT(*) FROM attendance_sessions asess
                 JOIN courses c3 ON asess.course_id = c3.id
                 WHERE c3.teacher_id = :teacher_id AND asess.is_active = 1) as total_sessions
              FROM courses c 
    ...
    $stmt->execute();
    $teacher_stats = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    error_log('Teacher stats error: ' . $e->getMessage());
    $teacher_stats = ['total_courses' => 0, 'total_students' => 0, 'total_sessions' => 0];
}
...
<div class="row justify-content-center">
    <div class="col-lg-8">
        ...
        <div class="row mb-4">
            <div class="col-md-4 mb-3">
                <div class="stats-card">
                    <h3><?php echo $teacher_stats['total_courses']; ?></h3>
                    <p class="mb-0">Toplam Dersim</p>
                </div>
            </div>
            <div class="col-md-4 mb-3">
                <div class="stats-card">
                    <h3><?php echo $teacher_stats['total_students']; ?></h3>
                    <p class="mb-0">Toplam Öğrencim</p>
                </div>
            </div>
            <div class="col-md-4 mb-3">
                <div class="stats-card">
                    <h3><?php echo $teacher_stats['total_sessions']; ?></h3>
                    <p class="mb-0">Yoklama Oturumum</p>
                </div>
            </div>
        </div>
        ...
        <div class="form-section">
            <h5 class="section-title">
                <i class="fas fa-phone me-2"></i>Telefon Numarası Güncelle
            </h5>
            ...
            <form method="POST">
                <div class="row align-items-end">
                    ...
                    <div class="col-md-4">
                        <button type="submit" name="update_phone" class="btn btn-success w-100">
                            <i class="fas fa-save me-1"></i>Güncelle
                        </button>
                    </div>
                </div>
        ...
        <div class="form-section">
            ...
            <div class="alert alert-info mt-3">
                ...
                <strong>Not:</strong> Ad soyad değişiklikleri için sistem yöneticinize başvurun.
            </div>
        </div>
